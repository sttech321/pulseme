                              ...
                              <tr data-v-f15ab7a3="" data-v-428084ba="" class="flex w-full">
                                 ...
                                 <td data-v-f15ab7a3="" class="p-10px w-200px">
                                    <div id="<?= $review['ID']; ?>" class="flex flex-col items-center justify-center h-full" data-id="<?= $review['ID']; ?>" data-approved="<?= $review['isApproved']; ?>"
                                       ...
                                       <button class="btn btn-approve w-full mb-5px <?= $review['isApproved'] == '1' ? 'btn-gray' : 'btn-green' ?>" data-archive="<?= $review['isArchive'] ?>" approved="<?= $review['isApproved']; ?>" onclick="handleApprovalClick(this,'approved')">
   ...
   <script>
      $(document).ready(function() {
         ...
         $(document).on('click', '#reset', function() {
            ...
            var inputValue = $('#review_name').val();
            $('#mySelect').append('<option>' + inputValue + '</option>');
                   //  $('#mySelect').append('<option>' + inputValue + '</option>');
                  ...
                  if (Array.isArray(data.reviews) && data.reviews.length > 0) {
                     ...
                     data.reviews.forEach(function(review) {
                        ...
                        var newRow =
                           ...
                           '</div>' +
                           '<div class="info-tag bg-white opacity-40 py-5px px-10px rounded-full m-5px shadow border">' +
                           '<p><span class="font-bold" id="">Campaign:</span>' + review.name + '</p>' +
                           '</div> <input type="hidden" id="review_name" value="' + review.name + '">' +
                           '<div class="info-tag bg-white opacity-40 py-5px px-10px rounded-full m-5px shadow border">' +
                           ...
                           '<div class="grid grid-cols-3 gap-20px text-left mb-20px">' +
                           '<div class="flex flex-col items-stretch col-span-3">' +
                           '<select class="outline-none py-7px border-b focus:border-blue-500" name="campaign" id="mySelect" aria-label=".form-select-lg example">' +
