	...
	<head>
		...
		<script>
			(function(w) {
			        ...
			        if (typeof _j.identity === 'undefined') {_j.identity ={"id": 15407, "location_id": 4288, "role_name": "user", "subscribed": ["pulseM", "contact-card"], "username": "st.tech321@gmail.com", "version": "v50.0.7"};}
			        if (typeof _j.account_meta === 'undefined') {_j.account_meta ={"accepted_tos": 1, "account_color": "#FFFFFF", "account_owner_email": "caitlyn.seidel@halesac.com", "account_owner_name": "Hales AC Service", "account_owner_phonenumber": "727-263-2084", "account_pulsem_phonenumber": "+1-844-478-5736", "address": "4700 95Th St N", "affiliation_type": null, "api_type": "successware", "city": "Saint Petersburg", "compliance": [], "country": "US", "default_campaign_uid": "1B7EFFD223291", "first_visit": 0, "google_meta_tag": null, "industry": "homeservices", "is-home-services": 1, "is-roofing": 0, "is_active": 1, "is_home_services": 1, "is_integrated": 1, "is_onboarded": 0, "is_roofing": 0, "navlogourl": "https://kiliassets.speetra.com/prod/account_images/15407/logo/1709862934_original.png", "state": "FL", "timezone": "US/Eastern", "username": "17790667675-caitlyn.seidel@halesac.com", "vendor": null, "web_url": "https://www.halesac.com/", "zipcode": "33708-3728"};}
			        ...
			        if (typeof _j.version === 'undefined') {_j.version = "v50.0.7"}
		...
		<script>
			/**
			 * For experimentation purposes. Will disable the social media modal, sm buttons will link directly.
			 * @namespace {boolean} DISABLE_SM_MODAL
			 */
			window.DISABLE_SM_MODAL = undefined

			window.page_prefix = window.location.href.replace(/[^/]*$/, '').replace(/\/?[^/]*$/, '').replace(/[^/]*$/, '')
			window.GA_INIT = window.GA_INIT || {}
			window.GA_INIT = {
			    delay_loader_hide: true,
			...
			try {
			    ...
			    window.dataLayer.push({ 'pulsem_account_id': String(meta.pulsem_account_id) })
			    window.dataLayer.push({ 'flow_mode': String(meta.flow_mode) })
			} catch(e) {
			    console.error(e)
			}
		</script>
	...
	<body>
		<div id="overlay-loader" style="display: none;">
			...
			<div class="preloader-container">
				...
				<div class="preloader pl-xxl pls-white">
		...
		<script>
			(function() {
			    ...
			    /**
			     * Install page-hider for Optimize variants
			     */
			    if (window.GA_INIT && window.GA_INIT.delay_loader_hide) {
			        var hideFn,
			            timeout = 4000,
			            className = 'async-show',
			            hideConfig = { 'GTM-T85VDBW': true },
			            loaderElem = document.getElementById('page-loader')

			        loaderElem.className += ' ' + className

			        hideConfig.start = 1 * new Date
			        hideConfig.end = hideFn = function() {
			            setTimeout(function() {
			                loaderElem.className = loaderElem.className.replace(RegExp(' ?' + className), '')
			            }, 600);
			        };

			        (window['dataLayer'] = window['dataLayer'] || []).hide = hideConfig
			        setTimeout(function() {
			            hideFn()
			            hideConfig.end = null
			        }, timeout)
			        hideConfig.timeout = timeout
			    }


			    /** @function ga
			     *  @namespace {Object} dataLayer
			     */
			    ...
			    ga('create', 'UA-40117151-7', 'auto')  // Connect to Kilimanjaro property
			    ga('require', 'GTM-T85VDBW')  // Connect to Kilimanjaro container

			    /* Require linkId so that link tracking is more robust. (needs links to have different id's) */
			    ga('require', 'linkid')

			    /* Send page view.
			     * Page can be customized through global variable 'GA_INIT.set_page'
			     */
			    if (window.GA_INIT && window.GA_INIT.set_page) {
			        ga('set', 'page', window.GA_INIT.set_page)
			    }
			    ga('send', 'pageview')

			    if (/localhost|adhoc-nfasjg3o1w|colorado|hawaii|texas|dev|/gi.test(location.hostname)) {
			        window.ga_debug = { trace: true }
			    }

			    delete window.GA_INIT
			})()
		</script>

		<div class="container">
			<div id="video-container" class="p-t-10">
				<div id="video-input">
					<label id="upload-campaign-img" class="btn btn-lg btn-icon-text btn-primary waves-effect m-10">
						<i class="zmdi zmdi-videocam"></i>
						<span>Upload or Take Video</span>
						<input type="file" accept="video/mp4,video/x-m4v,video/*;capture=camcorder" data-mycloudinary-upload="video-testimonial" />
					</label>

					<button class="btn btn-success btn-lg btn-icon-text waves-effect m-10 hide" data-video-input="upload">
				...
				<div id="player-controls">
					<div class="text-center">
						<button class="btn btn-success btn-lg btn-icon-text waves-effect m-10 hide" data-recorder="upload">
							<i class="zmdi zmdi-cloud-upload"></i>
							<span>Upload Recording</span>
						</button>
						<button class="btn btn-primary btn-lg btn-icon-text waves-effect m-10 hide" data-recorder="start">
							<i class="zmdi zmdi-videocam"></i>
							<span>Record Me</span>
						</button>
						<button class="btn btn-danger btn-lg btn-icon-text waves-effect m-10 hide" data-recorder="stop">
							<i class="zmdi zmdi-stop"></i>
							Stop
						</button>
						<div data-recorder="loading" class="p-t-10 p-b-10">
							<div class="preloader pl-xs m-r-5" style="display: inline-block;">
								<svg class="pl-circular" viewBox="25 25 50 50">
									<circle class="plc-path" cx="50" cy="50" r="20"></circle>
								</svg>
							</div>
							<h4 style="display: inline-block; line-height:20px; vertical-align:top; margin:0;">Loading camera...</h4>
						</div>
					</div>
				</div>
				<div id="player-error" class="hide" class="p-l-20 p-r-20">
					...
					<button id="ask-permission" class="btn btn-primary waves-effect hide">Grant Permission</button>
				</div>
			</div>

			<div class="wrapper test-transaction has-bottom-logo">
				<div class="card text-center">
					<div id="pulsecheck-title" class="card-header row m-0" style="background-color: #FFFFFF;">
						<div class="col-xs-6">
					...
					<div class="flow-card social-media" data-flow-id="20" data-flow-back="1" data-type="social-media">
						...
						<div class="social-media-websites positive" data-is-last="0" data-valid="True" data-confirm-text="I left a great review!" data-success="21" data-failure="22">
							...
							<a
								href="https://www.facebook.com/login/?next=https%3A//www.facebook.com/pg/None/reviews/"
								target="_blank"
								data-sm="facebook"
								class="social-media-btn sm-banner facebook btn btn-block btn-lg"
								data-success="21"
								data-failure="22"
								data-retry="1"
								data-app="facebook"
					...
					<div class="flow-card social-media" data-flow-id="60" data-flow-back="1" data-type="social-media">
						<div class="flow-card-header clearfix">
							<h2 class="flow-card-title">
								Thank you!<br />
						...
						<div class="social-media-websites" data-is-last="0" data-valid="True" data-confirm-text="Yes I did" data-success="61" data-failure="62">
							<a href="javascript:void(null);" class="social-media-btn sm-banner contact-us btn btn-block btn-lg" data-sm-skip="62">
					...
					<div class="flow-card thank-you" data-flow-id="24" data-flow-back="None" data-type="thank-you">
						<div class="flow-card-header clearfix"></div>
						<div class="p-20">
							<h3 class="text-center m-t-0 m-b-25" data-text-type="POSITIVE-BASIC">
								We enjoyed your business too!
							</h3>
							<h4>Liked the service? Don't forget to tell your friends!</h4>
						</div>
					</div>

					<div class="flow-card thank-you" data-flow-id="44" data-flow-back="None" data-type="thank-you">
						<div class="flow-card-header clearfix"></div>
						<div class="p-20">
							<h3 class="text-center m-t-0 m-b-25" data-text-type="NEUTRAL-BASIC">
					...
					<div class="flow-card thank-you" data-flow-id="64" data-flow-back="None" data-type="thank-you">
						<div class="flow-card-header clearfix"></div>
						<div class="p-20">
...
let currentStep = 0;
	...
	function updateUrlStep(step) {
		// Update the URL with the new step without reloading the page
	...
	function getStepFromUrl() {
		...
		return parseInt(params.get('step'), 10) || 0; // Default to step 0 if not set
	}
	...
	function showStep(step) {
		...
		updateUrlStep(step); // Update the URL when showing a step
	}
	...
	function nextpart() {
		// Directly activate step 5 (index 4)
		currentStep = 4;
		showStep(currentStep);
		console.log(currentStep);
	}

	// Initialize the form step from the URL on page load
